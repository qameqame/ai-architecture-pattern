...
Parallelizationは、複数のLLM呼び出しを順番にではなく同時に実行するパターンです。処理全体の待ち時間を短縮する、あるいは1回の試行だけに頼らず複数の独立した試行をサンプリングして結果の頑健性を高める、といった目的で使われます。このアイデアをベースにした2つの代表的な派生パターンがあります。
...
flowchart LR
    A[問い合わせ] --> B{分類器}
    B -- billing --> C[請求担当ハンドラー]
    B -- technical --> D[技術サポートハンドラー]
    B -- account --> E[アカウント担当ハンドラー]
    B -- other --> F[一般担当ハンドラー]
```
...
```mermaid
flowchart LR
    A[トピック] --> B["Orchestrator: 計画<br/>(セクション一覧、動的に決定)"]
    B --> C1[Worker 1]
    B --> C2[Worker 2]
    B --> C3[Worker 3]
    B --> C4[Worker 4]
    C1 --> D["Orchestrator: 統合"]
    C2 --> D
    C3 --> D
    C4 --> D
    D --> E[最終レポート]
