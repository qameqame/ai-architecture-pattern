...
「行動喚起の有無」をチェック。不合格ならフィードバックを付けて再生成。
...
def evaluate_copy(topic: str, copy_text: str) -> EvalResult:
    """プログラム側で機械的にチェックできる基準(文字数/キーワード)はコードで判定し、
    トーンのようなLLMでないと判断しづらい基準だけLLMに評価させる。
    """
    issues = []
    if len(copy_text) < 20:
    ...
    if "今すぐ" not in copy_text and "ぜひ" not in copy_text:
        issues.append("行動を促す表現('今すぐ'や'ぜひ'等)がありません。")

    # トーンのチェックはLLM評価者に任せる
    tone_prompt = (
        f"次の広告コピーは前向きで魅力的なトーンになっていますか？"
        f"「はい」か「いいえ」のどちらか一語だけで答えてください。\n\n{copy_text}"
    )
    tone_ok = "はい" in ollama_chat(tone_prompt, temperature=0.0)
    if not tone_ok:
